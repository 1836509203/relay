...
public class LocalProcess {
    ...
    private func enqueueReceivedData(_ bytes: [UInt8]) {
        pendingLock.lock()
        // Relay patch: PTY floods arrive as tiny reads (tens of bytes); one
        // queued array per read means hundreds of thousands of chunks per
        // megabyte through the drain/feed path, and a deep backlog while the
        // reader outpaces the main thread. Coalesce into the newest chunk
        // that drainReceivedData has not picked up yet, capped at 64KB.
        if let last = pendingChunks.indices.last, last >= pendingChunkIndex,
           pendingChunks[last].count + bytes.count <= 65536 {
            pendingChunks[last].append(contentsOf: bytes)
        } else {
    ...
    var totalRead = 0
    // Relay patch: bytes delivered by the read operation currently in flight
    // (readQueue is serial, no synchronization needed). Lets the EOF check
    // below tell a by-length completion apart from a genuine EOF.
    var currentOpBytes = 0
    func childProcessRead (done: Bool, data: DispatchData?, errno: Int32) {
        ...
        if data.count == 0 {
            // Relay patch: an operation that already delivered data and then
            // finishes with an empty done callback has merely exhausted its
            // readSize budget — by-length completion, NOT EOF. Whether the
            // final bytes ride on the done callback or arrive in an earlier
            // partial callback is a race inside DispatchIO; treating the
            // empty variant as EOF set running=false and silently killed
            // keyboard input on busy sessions. A real EOF still gets caught:
            // the chained read completes empty with currentOpBytes == 0 and
        ...
        // Relay patch: a live DispatchIO stream read keeps delivering partial
        // data (done == false) on its own; only the completed operation needs
        // a follow-up read. Re-issuing on every partial callback stacks up
        // pending operations without bound (~530 bytes each, never released
        // while the PTY stays open) and fragments output into byte-sized
