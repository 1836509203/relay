...
final class MetalTerminalRenderer: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        // Relay patch: the CAMetalLayer must stay non-opaque, or the
        // premultiplied clear below ignores destination alpha at composite
        // time and a transparent terminal background shows as solid black.
        // Re-assert per frame: the layer can be (re)created lazily by AppKit
        // after the one-shot configuration in updateMetalRenderer ran.
        ...
        // Relay patch: CAMetalLayer composites premultiplied — clearing with
        // straight (rgb, a<1) would add the full rgb over the backdrop.
        // Premultiply so a transparent background clears to (0,0,0,0).
        let bgA = Double(bgColor.w)
        passDescriptor.colorAttachments[0].clearColor = MTLClearColorMake(Double(bgColor.x) * bgA,
                                                                         Double(bgColor.y) * bgA,
                                                                         Double(bgColor.z) * bgA,
                                                                         bgA)
    ...
    private func buildRowDrawData(row: Int,
        ...
        for shaped in shapedSegments {
            // Relay patch: column accounting must follow source CHARACTERS,
            // not glyph counts — ligatures/cluster shaping can make the two
            // diverge, and the glyph pass anchors by character ordinal.
            ...
            for run in shaped.runs {
                ...
                let startColumn = shaped.segment.column + (processedChars * shaped.segment.columnWidth)
                let endColumn = startColumn + (run.shaperRun.charCount * shaped.segment.columnWidth)
                ...
                processedChars += run.shaperRun.charCount
            }
        }
        ...
        for shaped in shapedSegments {
            ...
            for run in shaped.runs {
                ...
                // Relay patch: anchor every grapheme cluster at its own grid
                // column (baseX + ordinal × columnStep + intra-cluster offset)
                // instead of translating the shaper's typographic positions by
                // a single run offset — fallback-font advances drift off the
                // cell grid and accumulate over long CJK runs.
                let columnStep = cellWidth * CGFloat(shaped.segment.columnWidth)
                ...
                for glyphRun in run.shaperRun.glyphRuns {
                    ...
                    for i in 0..<glyphRun.glyphs.count {
                        ...
                        let gridX = baseX + columnStep * CGFloat(glyphRun.charOrdinals[i])
                        let basePos = CGPoint(x: gridX + glyphRun.clusterDx[i],
                ...
                if let rawStyle = runAttributes[.underlineStyle] as? Int,
                   rawStyle != 0 {
                    ...
                    for (j, ctPos) in run.shaperRun.positions.enumerated() {
                        let gridX = baseX + columnStep * CGFloat(run.shaperRun.flatOrdinals[j])
                        let basePos = CGPoint(x: gridX,
                                              y: lineOrigin.y + yOffset + ctPos.y)
                        let x0 = basePos.x * scale
                        let x1 = (basePos.x + max(decorationCellWidth, columnStep)) * scale
    ...
    private struct ShaperGlyphRun {
        ...
        // Relay patch: per-glyph source-character ordinal in the shaped text
        // plus the glyph's x offset inside its grapheme cluster. Glyphs must
        // be anchored at their own terminal-grid column — the shaper's
        // typographic advances drift off the grid (CJK fallback fonts advance
        // 12pt where the grid expects 2 cells ≈ 14.5pt), so long runs render
        // compressed, leave gaps at run boundaries, and shift when selection
        // re-splits the attribute runs.
        let charOrdinals: [Int]
        let clusterDx: [CGFloat]
    }
    ...
    private struct ShaperRun {
        ...
        let glyphCount: Int
        let firstX: CGFloat
        // Relay patch: flat variant of charOrdinals aligned with `positions`,
        // and the shaped text's Character count for grid column accounting.
        let flatOrdinals: [Int]
        let charCount: Int
    }
    ...
    private final class ShaperCache {
        ...
        func shape(text: String, font: CTFont) -> ShaperRun? {
            ...
            // Relay patch: map UTF-16 offsets to Character ordinals so each
            // glyph can be tied back to the cell of the character that
            // produced it (surrogate pairs and combining marks share the
            // ordinal of their grapheme cluster).
            var ordinalOf = [Int](repeating: 0, count: text.utf16.count + 1)
            var ord = 0
            var u16 = 0
            for ch in text {
                for k in 0..<ch.utf16.count {
                    ordinalOf[u16 + k] = ord
                }
                u16 += ch.utf16.count
                ord += 1
            }
            ordinalOf[text.utf16.count] = ord
            var clusterFirstX: [Int: CGFloat] = [:]

            var glyphRuns: [ShaperGlyphRun] = []
            var positions: [CGPoint] = []
            var flatOrdinals: [Int] = []
            ...
            for run in runs {
                ...
                var ords: [Int] = []
                var dxs: [CGFloat] = []
                ords.reserveCapacity(count)
                dxs.reserveCapacity(count)
                for i in 0..<count {
                    let o = ordinalOf[max(0, min(stringIndices[i], ordinalOf.count - 1))]
                    let anchor = clusterFirstX[o] ?? runPositions[i].x
                    if clusterFirstX[o] == nil {
                        clusterFirstX[o] = anchor
                    }
                    ords.append(o)
                    dxs.append(runPositions[i].x - anchor)
                }
                ...
                glyphRuns.append(ShaperGlyphRun(font: runFont, glyphs: glyphs, positions: runPositions,
                                                charOrdinals: ords, clusterDx: dxs))
                positions.append(contentsOf: runPositions)
                flatOrdinals.append(contentsOf: ords)
            }
    ...
    private func buildCursorDrawData(scale: CGFloat,
        ...
        #if os(macOS)
        // Relay patch: IME 组合期插入点由 caretView 在预编辑文本内表示，
        // 终端自身光标（停在组合起点）此时绘制只会误导（图51）。
        if !terminalView.imeMarkedText.isEmpty {
            return ([], [], [])
        }
        #endif
