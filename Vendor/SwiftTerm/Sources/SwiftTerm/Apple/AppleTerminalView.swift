...
extension TerminalView {
    ...
    /// Relay patch: extra points added to every computed cell height —
    /// a visual line-spacing knob (Ghostty's adjust-cell-height). Set it
    /// before (re)assigning `font`; the font setter recomputes dimensions.
    public static var cellHeightAdjustment: CGFloat = 0

    /// Relay patch: extra points added to the computed cell width
    /// (Ghostty's adjust-cell-width). Negative tightens letter spacing;
    /// CJK occupies 2 cells so it tightens twice as much per glyph.
    public static var cellWidthAdjustment: CGFloat = 0
    ...
    func computeFontDimensions () -> CellDimension
    {
        ...
        // Relay patch: no ceil-to-whole-points here — the pixel-grid snap
        // below is the only rounding needed. Pre-ceiling inflated the line
        // pitch by up to a full point (SF Mono 12pt: 14.13 → 15.0 instead
        // of 14.5 on a 2x display), which reads as extra line spacing.
        // Relay patch: + cellHeightAdjustment = line-spacing knob.
        let cellHeight = lineAscent + lineDescent + lineLeading + TerminalView.cellHeightAdjustment
        #if os(macOS)
        ...
        // }
        // Relay patch: map "W" through the cmap (CTFontGetGlyphsForCharacters)
        // instead of glyph(withName:). CID-keyed OTFs (Noto Sans Mono CJK SC,
        // Source Han Mono, …) have no "W"-named glyph — the name lookup
        // returns .notdef whose advance is the full-width em (12pt @12pt),
        // doubling the cell width and letterspacing all ASCII text.
        let ctFont = fontSet.normal as CTFont
        var wChar: [UniChar] = [0x57] // "W"
        var wGlyph: [CGGlyph] = [0]
        var cellWidth: CGFloat
        if CTFontGetGlyphsForCharacters(ctFont, &wChar, &wGlyph, 1), wGlyph[0] != 0 {
            var adv = CGSize.zero
            CTFontGetAdvancesForGlyphs(ctFont, .horizontal, wGlyph, &adv, 1)
            cellWidth = adv.width
        } else {
            let glyph = fontSet.normal.glyph(withName: "W")
            cellWidth = fontSet.normal.advancement(forGlyph: glyph).width
        }
        #else
        let fontAttributes = [NSAttributedString.Key.font: fontSet.normal]
        let cellWidth = "W".size(withAttributes: fontAttributes).width
        #endif
        // Relay patch: + cellWidthAdjustment = letter-spacing knob (negative
        // tightens; CJK at 2 cells shrinks twice as much — the point).
        cellWidth += TerminalView.cellWidthAdjustment
    ...
    func mapColor (color: Attribute.Color, isFg: Bool, isBold: Bool, useBrightColors: Bool = true) -> TTColor
    {
        ...
        case .defaultInvertedColor:
            if isFg {
                ...
                // Relay patch: reverse-video background must stay visible even
                // when the default background is transparent (translucent
                // window mode sets nativeBackgroundColor alpha to 0).
                return nativeBackgroundColor.inverseColor().withAlphaComponent(1)
            }
    ...
    func updateCursorPosition()
    {
        ...
        #endif
        var caretOrigin = CGPoint(x: lineOrigin.x + (cellDimension.width * doublePosition * CGFloat(buffer.x)), y: lineOrigin.y)
        #if os(macOS)
        // Relay patch: IME 组合期 caretView 充当预编辑插入点，跟在
        // selectedRange 位置（拼音=末尾）；不加偏移会停在组合起点。
        caretOrigin.x += imeCaretOffsetX()
        #endif
        caretView.frame.origin = caretOrigin
    ...
    {
        #if os(macOS)
        // Relay patch: coalesced from scrolled(source:yDisp:) — one scroller
        // and delegate update per display pass instead of per scrolled line.
        ...
        if TerminalView.displayStatsOn {
            ...
            TerminalView.statDisplayPasses += 1
            TerminalView.statDisplayNs += t1 - t0
            if t1 - TerminalView.statDisplayLastPrint > 2_000_000_000 {
                TerminalView.statDisplayLastPrint = t1
                FileHandle.standardError.write(Data(
                    "[dispstat] passes=\(TerminalView.statDisplayPasses) display=\(TerminalView.statDisplayNs / 1_000_000)ms\n".utf8))
            }
        } else {
            updateDisplay (notifyAccessibility: true)
        }
    ...
    // Relay patch: RELAY_IO_STATS=1 display-pass instrumentation.
    static let displayStatsOn = ProcessInfo.processInfo.environment["RELAY_IO_STATS"] == "1"
    static var statDisplayPasses = 0
    static var statDisplayNs: UInt64 = 0
    static var statDisplayLastPrint: UInt64 = 0
    ...
    func feedPrepare()
    {
        ...
        // 会在备用屏（Claude Code/codex 等全屏 TUI）里把用户按 ⌥ 划下的选区每帧抹掉。
        // 与 linefeed 对齐：备用屏坐标稳定、输出刷新不清选区；主屏维持原行为。
