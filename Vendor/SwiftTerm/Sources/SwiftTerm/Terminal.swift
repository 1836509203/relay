...
open class Terminal {
    ...
    /// Relay patch: the valid row range for getScrollInvariantLine. Rows are
    /// absolute since session start — linesTop grows as old lines are
    /// trimmed, so iterating from 0 misses everything on long sessions.
