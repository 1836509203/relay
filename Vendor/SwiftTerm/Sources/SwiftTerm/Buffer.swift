...
//                #endif
    ...
    public func resize (newCols : Int, newRows : Int)
    {
        ...
        if lines.count > 0 {
            ...
            if cols < newCols {
                // Relay patch: iterate count, not maxLength. The CircularList
                // subscript materializes nil slots via makeEmpty, so walking
                // maxLength turns the entire scrollback capacity into real
                // BufferLines (~3KB each; 10k scrollback ≈ +30MB per resize,
                // even on idle sessions). Slots beyond count don't exist yet;
                // makeEmpty creates them at the Buffer's current cols later.
                for i in 0..<lines.count {
        ...
        if isReflowEnabled {
            ...
            // Relay patch: count, not maxLength (see the matching loop above —
            // walking maxLength materializes every empty scrollback slot).
            if cols > newCols {
                for i in 0..<lines.count {
                    lines [i].resize (cols: newCols, fillData: CharData.Null)
                }
        ...
        // Relay patch: upstream's DEBUG post-condition walked 0..<maxLength,
        // which both materialized the whole scrollback capacity and called
        // abort() in production. Removed.
        rows = newRows
        cols = newCols
    }
